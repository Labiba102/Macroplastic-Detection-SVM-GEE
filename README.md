...
* `src/01_SVM_Water_Extraction.js`: Training logic and morphological smoothing for water channel detection.
* `src/02_Building_Mask_Integration.js`: Data fusion logic using building footprints for ROI refinement.
* `src/03_Plastic_Analysis_WPIS.js`: Spectral index calculation (PI) and statistical evaluation of pollution intensity.
...
2.  Load the scripts from the `/src` folder.
3.  Set your desired **ROI** and adjust the **Dynamic Threshold Sliders** to visualize plastic accumulation hotspots.
